...
bool SmoothTool::draw_object_properties() {
	...
	if(ImGui::Button("Smooth##btn_smooth_smooth_tool", ImVec2(ImGui::GetContentRegionAvail().x, 0))) {
		...
		// Convert vector to attribute
		// CellFacetAttribute<int> emb_attr(ctx.hex_bound->hex, -1);
		// emb_attr.ptr->data = ctx.emb;
		...
		write_by_extension("smoothed.geogram", ctx.hex_bound->hex);
